...
class _IREditScreenState extends State<IREditScreen> {
  ...
  final Set<String> _recipientEmails = {};
  ...
  Future<void> _generatePDFPreview() async {
    ...
    try {
      ...
      final pdfPath = await _reportService.generatePDFFromIR(
        updatedData,
  ...
  Future<String?> _getSignatureImagePath() async {
    ...
    final imagePath = join(
      directory.path,
      'signature_${FirebaseAuth.instance.currentUser?.uid}.png',
    );
  ...
  Future<String?> _getSealImagePath() async {
    ...
    final imagePath = join(
      directory.path,
      'seal_${FirebaseAuth.instance.currentUser?.uid}.png',
    );
    final file = File(imagePath);
    if (await file.exists()) {
      return imagePath;
  ...
  Future<void> _saveSignatureImage(Uint8List bytes) async {
    final directory = await getApplicationDocumentsDirectory();
    final imagePath = join(
      directory.path,
      'signature_${FirebaseAuth.instance.currentUser?.uid}.png',
  ...
  Future<void> _saveSealImage(File imageFile) async {
    ...
    try {
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text('Seal saved successfully'),
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text('Error saving seal: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
                              ...
                              children: [
                                TextFormField(
                                  ...
                                  decoration: _buildInputDecoration(
                                    label: 'Supplier Delivery Note/Date',
                                  ),
                                ),
                                const SizedBox(height: 16),
                                _buildArabicTextField(
                            ...
                            child: Column(
                              ...
                              children: [
                                DropdownButtonFormField<String>(
                                  ...
                                  onChanged:
                                      (value) =>
                                          _updateIRData(masStatus: value),
                                ),
                                const SizedBox(height: 16),
                                DropdownButtonFormField<String>(
                                  value:
                                      _irData.dtsStatus.isEmpty
                                          ? null
                                          : _irData.dtsStatus,
                                  decoration: _buildInputDecoration(
                                    label: 'DTS Status',
                                  ...
                                  onChanged:
                                      (value) =>
                                          _updateIRData(dtsStatus: value),
                                ),
                                const SizedBox(height: 16),
                                DropdownButtonFormField<String>(
                                  value:
                                      _irData.dispatchStatus.isEmpty
                                          ? null
                                          : _irData.dispatchStatus,
                                  decoration: _buildInputDecoration(
                                  ...
                                  onChanged:
                                      (value) =>
                                          _updateIRData(dispatchStatus: value),
                                ),
                              ],
                            ),
                          ),
  ...
  Future<void> _loadSignatureImage() async {
    // Implementation of loading existing signature image
  }

  Future<void> _loadSealImage() async {
    // Implementation of loading existing seal image
  }
  ...
  Future<void> _saveSignature(dynamic data) async {
    ...
    try {
      ...
      setState(() {
        _signatureImagePath = imagePath; // Update the displayed image path
      });
      scaffoldMessenger.showSnackBar(
